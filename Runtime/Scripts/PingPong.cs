...
//{
    public class PingPong : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            Vector3 rotationUpdate = new Vector3(
                InitialRoll + ((float) Mathf.Sin((Time.time + Initialoffset) * XOffset) * WaveSizeX * RotationMult),
                InitialYaw * RotationMult,
                InitialPitch + ((float) Mathf.Sin((Time.time + Initialoffset) * ZOffset) * WaveSizeZ * RotationMult));
            ...
            transform.localPosition = new Vector3(
                Initialx + ((float) Mathf.Sin((Time.time + Initialoffset) * XOffset) * WaveSizeX * PositionMult),
            ...
            transform.localScale = InitialScale + (new Vector3(1, 1, 1) *
                                                   (float) Mathf.Sin((Time.time * ScaleOffset) * WaveSizeX) *
                                                   ScaleMult);
